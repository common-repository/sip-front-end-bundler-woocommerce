			...
			if( isset($_POST['bundler']) && isset($_POST['bundler']['id']) ) {
				...
				if( isset($coupons) && !empty($coupons) ) {
					foreach( $coupons as $coupon ) {
						...
						$post_coupon = get_post( $coupon, 'ARRAY_A');
						$coupon_apply = $post_coupon['post_title'];
						if ($coupon_apply != "") {
							$woocommerce->cart->add_discount($coupon_apply);
						}
					}
				}
	...
	if( !empty($id) ) {
		...
		if( !empty($bundle) ) {
			...
			if( file_exists( SIP_FEBWC_DIR . "templates/template-{$template}.php" ) ) {
				...
				return "<strong>WooBundler:</strong> Template doesn't exists.";
			}
...
function sip_febwc_wp_register_scripts() {
	...
	wp_register_script( "woo_bundles_cookie-js", SIP_FEBWC_URL . 'assets/js/js.cookie.js', array('jquery'), '', true );
					...
					<table width="100%" id="offers" border="1">
						<thead>
							<tr>
								...
								<th width="5%"><i id="loader" style="display: none; background: url(<?php echo SIP_FEBWC_URL ; ?>assets/img/ajax-loader.GIF); width: 35px; height: 35px;"></i></th>
							</tr>
						</thead>
						<tbody>
							<tr>
...
function get_offers( $bundleid, $type = "minimum" ) {
	...
	if( $type == 'product-quantity' ) {
		foreach( $offers as $key => $offer ) {
			if( $offer["type"] == '2' ) {
				$goffer = array("id" => $offer["type-product"], "quantity" => $offer["value"], "discount" => $offer["discount"], "discount_type" => $offer["discount-type"], "applied" => false, "coupon" => $offer["coupon"], "css" => $offer["css"], "override" => ( isset($offer["override"]) && $offer["override"] == "on" ) ? true : false );
		...
		foreach( $offers as $key => $offer ) {
			if( $offer["type"] == '0' ) {
				...
				$goffer = array("id" => $key, "discount" => $offer["discount"], "discount_type" => $offer["discount-type"], "applied" => false, "coupon" => $offer["coupon"], "css" => $offer["css"], "override" => ( isset($offer["override"]) && $offer["override"] == "on" ) ? true : false );
		...
		foreach( $offers as $key => $offer ) {
			//if( (($override == true && isset($offer["override"]) && $offer["override"] == 'on') || $override == false) ) {
				...
				$goffer = array("id" => ( isset($offer["type-product"]) ? $offer["type-product"] : $key ), "discount" => $offer["discount"], "discount_type" => $offer["discount-type"], "applied" => false, "coupon" => $offer["coupon"], "type" => $offer["type"], "css" => $offer["css"], "override" => ( isset($offer["override"]) && $offer["override"] == "on" ) ? true : false );
...
function sip_febwc_admin_post_notices() {
  ...
  $is_all_post_type = ( 'edit.php' == $pagenow && 'sip-bundles' == isset( $_GET['post_type'] ) );
