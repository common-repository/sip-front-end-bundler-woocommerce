...
class Sip_Front_End_Bundler_WC_Admin_Post_Tab extends WP_List_Table
{
  ...
  function process_bulk_action()
  {
    ...
    if ('delete' === $this->current_action()) {
        ...
        foreach ($ids as $id) {
          ...
          if( count($coupon_ids) > 0 ){
            foreach ($coupon_ids as $coupon_id) {
              $wpdb->query( $wpdb->prepare( "DELETE FROM $wpdb->posts WHERE id IN($coupon_id->post_id)"));
            }
          }
        }
      } else {
      ...
      if (!empty($ids)) {
          $wpdb->query( $wpdb->prepare( "DELETE FROM $wpdb->posts WHERE id IN(%s)", $ids));
      }
    }
  }
  ...
  function prepare_items()
  {
    ...
    $querystr = $wpdb->prepare("
        ...
        ORDER BY %s %s
        LIMIT %d OFFSET %d" , $orderby, $order, $per_page, $paged
     );
