...
define( 'SIP_FEBWC_ADMIN_VERSION' , '1.0.3' );
...
class Sip_Front_End_Bundler_WC_Admin {
  ...
  public function __construct() {
    ...
    add_action( 'admin_notices', array( &$this, 'sip_febwc_admin_notice') );
    add_action( 'admin_enqueue_scripts', array( &$this, 'sip_febwc_admin_enqueue') );

    if ( get_option( 'woocommerce_enable_coupons' ) != 'yes' ) {
      add_action( 'admin_notices', array( $this, 'sip_febwc_coupon_enable' ), 2000 );
    }
  }
   ...
   * @since 1.0.0
   */
  public function sip_febwc_admin_enqueue( ) {
   ...
   */
  public function sip_febwc_coupon_enable() { ?>
    <div id="message" class="error">
      <p><?php printf( __( 'Front End Bundler uses WooCommerce coupons to create discounts. If you want to use live offers, please enable coupons %shere%s .', 'front-end-bundler' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=checkout' ) ) . '">', '</a>' ); ?></p>
    </div>
    <?php
  }
        ...
        <div class="notice">
          ...
          <table style="width:98.5%; margin-top:10px; border-spacing:0px;">
            <tr class="sip-notification-message">
              ...
              <td>
                <div class="title">Unlock <b>advanced features</b>, get the <b>PRO version</b> today</div>
                <div class="subbtitle">Extend standard plugin functionality with extra styles, product quantities, variable products support and more.</div>
              </td>
              <td>
                <div class="button_div">
                  ...
                  <p style="text-align:center;"><span class="dismiss"><?php printf(__('<a href="%1$s">Dismiss</a>'), 'admin.php?page=sip-bundler-settings&sip_febwc_nag_ignore=0'); ?></span></p>
                </div>
              </td>
            </tr>
          </table>
  ...
  public function sip_febwc_action_links( $links ) {
    ...
    $plugin_links = "";
    if( ! class_exists( 'SIP_Front_End_Bundler_WC_Pro' ) ) {
      $plugin_links[] = '<a target="_blank" href="https://shopitpress.com/plugins/' .SIP_FEBWC_PLUGIN_SLUG. '/?utm_source=wordpress.org&utm_medium=SIP-panel&utm_content=v'. SIP_FEBWC_VERSION .'&utm_campaign='.SIP_FEBWC_UTM_CAMPAIGN.'" style="color: red; font-weight: bold">' . __( 'Go Pro!', 'front-end-bundler' ) . '</a>';
    }

    $plugin_links[] = '<a href="' . admin_url( 'admin.php?page=sip-bundler-settings' ) . '">' . __( 'Settings', 'front-end-bundler' ) . '</a>';
  ...
  */
  ...
  <div class="sip-febwc-wrap wrap">
    <h2>SIP Front End Bundler WooCommerce</h2>

    <div class="sip-container">
      <h2 class="nav-tab-wrapper">
        <a class="nav-tab<?php if ( !isset( $_GET['action'] ) || ( 'bundles' == $_GET['action'] || 'delete' == $_GET['action']  || 'delete' == isset( $_GET['action2'] ) ) ) echo ' nav-tab-active'; ?>" href="admin.php?page=sip-bundler-settings&amp;action=bundles"><?php _e( 'Bundles', 'front-end-bundler' ); ?></a>
      ...
      <?php
        if ( !isset( $_GET['action'] ) || 'bundles' == $_GET['action'] || 'delete' == $_GET['action'] || 'delete' == isset( $_GET['action2'] ) ) {
