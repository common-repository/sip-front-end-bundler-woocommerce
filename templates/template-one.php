		...
		<?php if( array_filter($products) ) { ?>
			...
			<?php
				...
				foreach( $products as $product ) {
					...
					<script>
						jQuery(function($) {
							...
							$result = Cookies.get("one-<?php echo $product."-".$bundler->ID; ?>");
							if ( $result == "true" ) {
								$("#one-<?php echo $product."-".$bundler->ID; ?>").closest('.wrap.woos').addClass('actives');
							}

							$qty_value = Cookies.get("one-value-<?php echo $product."-".$bundler->ID; ?>");
							if ( $qty_value > 0 ) {
								$("#one-value-<?php echo $product."-".$bundler->ID; ?>").val($qty_value);;
							}

							$qty_total = Cookies.get("one-total-<?php echo $bundler->ID; ?>");
							if ( $qty_total > 0 ) {
								$("#one-total-<?php echo $bundler->ID; ?>").val($qty_total);;
							}

						});
					</script>
					<div class="woos wrap <?php echo $boxclass; ?>">
						<div class="woos-product<?php echo ( ($perqty) ? "" : " noqty" ); ?>">
							<div class="woos-quantity" id="one-<?php echo $product."-".$bundler->ID; ?>">
								<div class="field">
									<div class="woos woofour">Qty: </div>
									<div class="woos wooseven qtyinput">
										<input type="number" min="0" id="one-value-<?php echo $product."-".$bundler->ID; ?>" max="<?php echo $wbproduct->get_stock(); ?>" name="bundler[product][<?php echo $counter; ?>][quantity]" readonly class="woos-quantity-input<?php echo ( ($perqty) ? "" : " readonly" ); ?>" data-error="<?php echo $bundler->get_field("outstock"); ?>" />
									</div>
							...
							<div class="woos-product-details">
								...
								<div class="details">
									...
									?>
									<?php if( isset($product_metas["weight"]) && $product_metas["weight"] != ""){
											$product_metas_ = $product_metas["weight"] ;
										} else {
											$product_metas_ = "" ;
										}
									?>
									<span style="display: none;"><?php echo $product_metas_ ; ?></span>
								</div>
							</div>
				...
				<div class="woos woothree woos-total">
					...
					$bundler->quantity_input("one-total-".$bundler->ID);
